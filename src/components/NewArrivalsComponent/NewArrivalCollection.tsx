...
export default function NewArrivalCollection(props: any) {
    ...
    return ( 
        <div className='w-full flex items-center relative px-10 lg:px-24' > 
            {!props.collection ?
                <div onClick={() => scroll(-100)} style={{border: '1px solid #192F5D'}} className='w-8 h-8 lg:w-12 lg:h-12 absolute left-4 cursor-pointer flex items-center justify-center' >
                    <svg width="9" height="16" viewBox="0 0 9 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M8 1L1 8L8 15" stroke="#192F5D" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </div>
            :null} 
            <div key={props.index} className='w-full flex flex-col justify-center' >
                <p style={{fontSize: '32px', color: '#192F5D'}} className='font-Assistant-Bold mt-8 text-center' >{props.item.category}</p>
                <div className='w-full flex flex-row overflow-x-auto scroll_event ' ref={ref} >  
                    {props.item.items.map((item:any, index:any ) => {
                        return(
                ...
                <button style={{border: '1px solid #192F5D', color: '#192F5D'}} className=' mx-auto px-8 mt-8 rounded-md py-2 font-Assistant-Bold '>SEE ALL</button>
            </div>
            {!props.collection ?
                <div onClick={() => scroll(100)} style={{border: '1px solid #192F5D'}} className='w-8 h-8 lg:w-12 lg:h-12  absolute right-4 cursor-pointer  cursor-pointer flex items-center justify-center' >
                    <svg width="9" height="16" viewBox="0 0 9 16" fill="none" xmlns="http://www.w3.org/2000/svg">
