...
export default function AllNewProducts(props: any) { 
 
    const [items, setItem] = React.useState([] as any) 
    const [loading, setLoading] = React.useState(false);  

    React.useEffect(() => {
        fetch(`https://olokun-api.herokuapp.com/items/new-arrivals?limit=4`, {
            method: 'GET', // or 'PUT'
            headers: {
    ...
    return (
        <div className='pb-8 w-full' >  
            <div className='w-full flex flex-col items-center my-14 mt-20'>
                ...
                {!loading ?  
                    <div className='w-full px-4 h-auto lg:h-203px lg:px-10 grid grid-cols-1 gap-1 lg:grid-cols-4 my-10' >  
                    ...
                    <>
                        {items.map((item: any, index: any) => {
                            ...
                            if(item.items.length === 0){
                                ...
                                return(
                                        <NewArrivalCollection item={item} index={index}  option={true} array='/new-arrivals?limit=4' /> 
                                    )
                        })}  
                    </>
                }
